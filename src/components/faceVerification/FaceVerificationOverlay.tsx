...
export default function FaceVerificationOverlay({ avatar, onComplete, onClose }: Props) {
    ...
    const inspect = (now: number) => {
      ...
      if (result.faceLandmarks.length !== 1) {
        ...
        if (tooFar || tooClose || offCenter) { holdSinceRef.current = null; blinkPhaseRef.current = "WAITING_FOR_OPEN"; resetBaselineSampling(); publishGuidance(tooFar ? "Move closer to the camera." : tooClose ? "Move back slightly." : "Center your face in the frame.", true); problemSinceRef.current ??= now; }
          ...
          const baseline = baselineRef.current; const userYawDelta = -(metrics.yaw - baseline.yaw); const userPitchDelta = -(metrics.pitch - baseline.pitch); const blinkClose = Math.max(GUIDANCE.blinkMinimumClosure, baseline.eyeClosure + GUIDANCE.blinkClosureOffset); const blinkReopen = baseline.eyeClosure + GUIDANCE.blinkReopenOffset;
          let qualifies = false; let wrongDirection = false;
          if (challenge === "NEUTRAL") qualifies = Math.abs(userYawDelta) <= GUIDANCE.neutralYawTolerance && Math.abs(userPitchDelta) <= GUIDANCE.neutralPitchTolerance;
          if (challenge === "TURN_RIGHT") { qualifies = userYawDelta >= GUIDANCE.turnYawThreshold; wrongDirection = userYawDelta <= -GUIDANCE.turnYawThreshold; }
          if (challenge === "TURN_LEFT") { qualifies = userYawDelta <= -GUIDANCE.turnYawThreshold; wrongDirection = userYawDelta >= GUIDANCE.turnYawThreshold; }
          if (challenge === "LOOK_UP") { qualifies = userPitchDelta >= GUIDANCE.lookPitchThreshold; wrongDirection = userPitchDelta <= -GUIDANCE.lookPitchThreshold; }
          if (challenge === "LOOK_DOWN") { qualifies = userPitchDelta <= -GUIDANCE.lookPitchThreshold; wrongDirection = userPitchDelta >= GUIDANCE.lookPitchThreshold; }
          if (challenge === "BLINK") {
            ...
            qualifies = blinkPhaseRef.current === "DETECTED" && Math.abs(userYawDelta) <= GUIDANCE.neutralYawTolerance && Math.abs(userPitchDelta) <= GUIDANCE.neutralPitchTolerance;
          }
  ...
  return <div className="fixed inset-0 z-[100] bg-[#050807]/95 p-4 text-white backdrop-blur-sm"><button type="button" aria-label="Cancel face verification" onClick={() => void cancelAndClose()} className="absolute right-5 top-5 z-10 rounded-full border border-white/20 px-4 py-2 text-sm">Cancel</button><div className="mx-auto flex h-full max-w-2xl flex-col items-center justify-center">
    ...
    {["REQUESTING_CAMERA", "COUNTDOWN", "CAPTURING", "UPLOADING", "SUCCESS"].includes(state) && <div className="w-full text-center"><div className={`relative mx-auto aspect-[3/4] max-h-[70vh] max-w-md overflow-hidden rounded-3xl border-4 ${redFeedback ? "border-red-500 motion-safe:animate-pulse" : "border-teal-300"}`}><video ref={videoRef} muted playsInline className="h-full w-full object-cover -scale-x-100" />{state === "COUNTDOWN" && <div className="absolute inset-0 flex items-center justify-center bg-black/35 text-8xl font-bold">{countdown || ""}</div>}</div>{state === "SUCCESS" ? <h2 className="mt-5 text-xl font-semibold text-teal-300">Capture successful</h2> : <><p className="mt-5 text-lg font-semibold">{challenge ? labels[challenge] : "Preparing camera"}</p><p className="mt-2 text-sm text-white/70">{state === "UPLOADING" ? "Saving capture…" : guidance}</p><p className="mt-3 text-xs text-white/55">{Math.min(index + 1, 5)} / 5</p></>}</div>}
